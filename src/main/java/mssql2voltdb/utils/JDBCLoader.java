...
import mssql2voltdb.DataMeta.JDBCJSONConfig;
...
public class JDBCLoader implements BulkLoaderErrorHandler {
    ...
    public static class JDBCLoaderConfig extends CLIConfig {
        ...
        @Option(shortOpt = "f",desc = "the pathname of config file")
        String pathname="";
        ...
        public void printUsage() {
            ...
            System.out
                    .println("       jdbcloader [args] -f configPath");
            super.printUsage();
        }
    }
    ...
    public static void main(String[] args) throws IOException,
        ...
        JDBCLoaderConfig cfg = new JDBCLoaderConfig();
        if(args.length>0) {
            cfg.parse(JDBCLoader.class.getName(), args);
            if (!cfg.pathname.equalsIgnoreCase("")) {
                Gson gson = new Gson();
                String path = cfg.pathname;
                System.out.println(path);
                try {
                    InputStream in = new FileInputStream(path);
                    String config = IOUtils.toString(in, "utf-8");
                    JDBCJSONConfig jdbcjsonConfig = gson.fromJson(config, JDBCJSONConfig.class);
                    cfg = jdbcjsonConfig.toJDBCLoaderConfig();
                } catch (FileNotFoundException e) {
