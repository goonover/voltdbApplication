...
import mssql2voltdb.DataMeta.CSVJSONConfig;
...
/**
 * MyCSVLoader is a simple utility to load data from a CSV formatted file to a table.
 *
 ...
 *
 */
public class MyCSVLoader implements BulkLoaderErrorHandler {
     ...
     * report file name
     */
    static String pathReportfile = "MyCSVLoaderReport.log";
    /**
     * log file name
     */
    static String pathLogfile = "MyCSVLoaderLog.log";
    private static final VoltLogger m_log = new VoltLogger("MyCSVLoader");
    ...
    public static class CSVConfig extends CLIConfig {
        ...
        public void printUsage() {
            System.out
                    .println("Usage: MyCSVLoader [args] tablename");
            System.out
                    .println("       MyCSVLoader [args] -p procedurename");
            super.printUsage();
        }
    }

    /**
     * MyCSVLoader main. (main is directly used by tests as well be sure to reset statics that you need to start over)
     *
     * @param args
     * @throws IOException
    ...
    //TODO:如果要改为微服务形式，直接把main移出去作为一个函数即可
    public static void main(String[] args) throws IOException,
        ...
        if(args.length>0) {
            cfg.parse(MyCSVLoader.class.getName(), args);
        }else {
            Gson gson=new Gson();
            String path=System.getProperty("user.dir");
            path+="\\src\\main\\resources\\csvConfig.json";
            InputStream in=new FileInputStream(path);
            String config= IOUtils.toString(in,"utf-8");
            CSVJSONConfig csvjsonConfig=gson.fromJson(config,CSVJSONConfig.class);
            cfg=csvjsonConfig.toCSVConfig();
        }
        config = cfg;
        ...
        try {
            csvClient = MyCSVLoader.getClient(c_config, serverlist, config.port);
        } catch (Exception e) {
        ...
        try {
            ...
            long ackCount;
            long rowsQueued;
            final MyCSVLoader errHandler = new MyCSVLoader();
    ...
    private static void configuration() {
        ...
        pathInvalidrowfile = config.reportdir + "MyCSVLoader_" + insertProcedure + "_"
                + "invalidrows.csv";
        pathLogfile = config.reportdir + "MyCSVLoader_" + insertProcedure + "_"
                + "log.log";
        pathReportfile = config.reportdir + "MyCSVLoader_" + insertProcedure + "_"
                + "report.log";
    ...
    private void produceFiles(long ackCount, long insertCount) {
        ...
        try {
            ...
            out_reportfile.write("MyCSVLoader elaspsed: " + elapsedTimeSec + " seconds\n");
            long trueSkip;
            long totolLineCnt;
            long totalRowCnt;
            ...
            out_reportfile.write("Number of rows that could not be inserted: "
                    + m_errorCount + "\n");
            out_reportfile.write("MyCSVLoader rate: " + insertCount
